...
void UAVProcessor::newData( TYPECPX* IQsamples, int L , int sampleRate ) {
   ...
   // qDebug() << " shifting by p= " << p ;
    ...
    if( p >= 0 ) {
        TYPECPX* psamples = samples + chirp_length  - m_bandwidth/UAV_BITRATE  ; // one bit before
        char *msg = demod( psamples, msg_length-chirp_length  ) ;
        if( msg != NULL ) {
            ...
            printf("Power=%0.3f noise level=%0.6f,  frame=%s\n", v2, 20*log10(noise_only), msg  );
        } else {
            printf("non valid. Power=%f\n", signal_plus_noise );
        }
    } else {
        printf("Error \n");
    }
...
char*  UAVProcessor::demod( TYPECPX* psamples, int L ) {
        ...
        if( start == L ) return(NULL);
         ...
         rxmsg[len] = (char)0 ;
         while( symbol < s ) {
                int v = demodByte( psamples, &symbol,L);
                if( v < 0 ) {
                      return(NULL);
                }
                if( !frame && (v==(int)'[')) {
...
int  UAVProcessor::calc( float *pvmax ) {
     ...
     // calc IFFT( FFT( signal ) x conj( FFT(chirp)) )
      ...
      vmoy /= fft_length ;
      float ratio =  20*log10( vmax/vmoy ) ;
      if( pvmax != NULL ) {
           (*pvmax) = vmax ;
      }
